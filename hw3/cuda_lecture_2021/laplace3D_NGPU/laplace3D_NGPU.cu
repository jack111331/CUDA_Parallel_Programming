	  ...
	  float* phiT_old, float* phi0_new, float* C)
{
    ...
    float  t, l, c, r, b, f, bk;     // top, left, center, right, bottom, front, back
    float  diff; 
    int    site, skip;

    int Lx = blockDim.x*gridDim.x;     // sublattice size in each GPU 
    int Ly = blockDim.y*gridDim.y;
    int Lz = blockDim.z*gridDim.z;
    int x  = blockDim.x*blockIdx.x + threadIdx.x;    // position in the sublattice 
    ...
    // TODO front and back case consider
    if (x == 0) {     // left boundary of the sub-lattice 
      if (phiL_old != NULL) {      // if it is not the boundary of the entire lattice
        l = phiL_old[(Lx-1)+y*Lx];
        r = phi0_old[site+1];
      } 
      else           // if it is the boundary of the entire lattice 
        skip = 1;
    }
    else if (x == Lx-1) {
      if(phiR_old != NULL) {
...
int main(void)
{
 ...
 //  Get the input parameters.
    ...
    printf("  Solve Laplace equation on 3D lattice with boundary conditions\n");
