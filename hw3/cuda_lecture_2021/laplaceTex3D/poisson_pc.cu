...
__global__ void laplacian(float* phi_old, float* phi_new, float* C, bool flag)
{
    ...
    float  t, l, r, b, f, bk;     // top, left, right, bottom, front, back
    float  diff; 
    int    site, zm1, zp1, ym1, yp1, xm1, xp1;
    ...
    if((x == 0) || (x == Nx-1) || (y == 0) || (y == Ny-1) || (z == 0) || (z == Nz-1) ) {  
      ...
      if(flag) {
        ...
        t = phi_old[yp1]; 
        f = phi_old[zp1]; 
        phi_new[site] = (b+l+r+t+f+bk-1)/6.0;
      }
      else {
        bk = phi_new[zm1]; 
        b = phi_new[ym1]; 
        ...
        t = phi_new[yp1]; 
        f = phi_new[zp1]; 
        phi_old[site] = (b+l+r+t+f+bk-1)/6.0;
      }
...
//        printf("iteration = %d\n",iter);
      ...
      flops = 9.0*(Nx-2)*(Ny-2)*iter;// FIXME
...
//      }
    ...
    if((CPU==0)||(CPU==2)) {      // to compute the CPU solution 
      ...
      float t, l, r, b, f, bk;    // top, left, right, bottom, front, back
      int site, zm1, ym1, xm1, xp1, yp1, zp1;
        ...
        if(flag) {
          ...
          for(int x=0; x<Nx; x++) { 
            if(x==0 || x==Nx-1 || y==0 || y==Ny-1 || z==0 || z==Nz-1) {   
              ...
              if(x == Nx/2 && y == Ny/2 && z == Nz/2) {
                h_new[site] = (b+l+r+t+f+bk-1.0)/6.0;
              } else{
                h_new[site] = (b+l+r+t+f+bk)/6.0;
              }
          ...
          for(int x=0; x<Nx; x++) { 
            if(x==0 || x==Nx-1 || y==0 || y==Ny-1 || z==0 || z==Nz-1) {
              ...
              f = h_new[zp1]; 
              if(x == Nx/2 && y == Ny/2 && z == Nz/2) {
                h_old[site] = (b+l+r+t+f+bk-1.0)/6.0;
              } else{
                h_old[site] = (b+l+r+t+f+bk)/6.0;
              }
