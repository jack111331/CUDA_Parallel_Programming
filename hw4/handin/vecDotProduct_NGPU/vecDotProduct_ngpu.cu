...
// Device code
__global__ void VecDotProduct(const float* A, const float* B, float* C, int N)
{
    ...
    float temp = 0.0f;
    // collect each element out of grid
    while (i < N) {
        temp += A[i] * B[i];
...
//	cudaSetDevice(cpu_thread_id);
	...
	if(cpu_thread_id == 0) {
        ...
        VecDotProduct<<<blocksPerGrid, threadsPerBlock, sm>>>(d_A, d_B, d_C, N/NGPU);
