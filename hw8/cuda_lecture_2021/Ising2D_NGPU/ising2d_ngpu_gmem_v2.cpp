...
__global__ void
metro_gmem_odd(int *spin, int *spin_top, int *spin_bottom, int *spin_left, int *spin_right, float *ranf, const float B,
               const float T) {
    int x, y, parity;
    int i, io;
...
__global__ void
metro_gmem_even(int *spin, int *spin_top, int *spin_bottom, int *spin_left, int *spin_right, float *ranf, const float B,
                const float T) {
    int x, y, parity;
    int i, ie;
    int old_spin, new_spin, spins;
...
// test code start
        ...
        for (int i = 0; i < Ly; i++) {
            float *h, *d;
            h = h_rng + 0 * Lx + (1 * Ly + i) * Nx;
            d = d_rng[1] + i * Lx;
            cudaMemcpy(d, h, Lx * sizeof(float), cudaMemcpyHostToDevice);
        }
        int *dL_spin, *dR_spin, *dT_spin, *dB_spin, *d0_spin;
        d0_spin = d_spin[1];
        dL_spin = (1 == 0) ? d_spin[NGx - 1 + 0 * NGx] : d_spin[1 - 1 + 0 * NGx];
        dR_spin = (1 == NGx - 1) ? d_spin[0 + 0 * NGx] : d_spin[1 + 1 + 0 * NGx];
        dT_spin = (0 == 0) ? d_spin[1 + (NGy - 1) * NGx] : d_spin[1 + (0 - 1) * NGx];
        dB_spin = (0 == NGy - 1) ? d_spin[1 + (0) * NGx] : d_spin[1 + (0 + 1) * NGx];
        metro_gmem_even<<<blocks, threads >> > (d0_spin, dT_spin, dB_spin, dL_spin, dR_spin, d_rng[1], B, T);

        cudaSetDevice(Dev[0]);
        for (int i = 0; i < Ly; i++) {
            float *h, *d;
            h = h_rng + 0 * Lx + (0 * Ly + i) * Nx;
            d = d_rng[0] + i * Lx;
            cudaMemcpy(d, h, Lx * sizeof(float), cudaMemcpyHostToDevice);
        }
        d0_spin = d_spin[0];
        dL_spin = (0 == 0) ? d_spin[NGx - 1 + 0 * NGx] : d_spin[0 - 1 + 0 * NGx];
        dR_spin = (0 == NGx - 1) ? d_spin[0 + 0 * NGx] : d_spin[0 + 1 + 0 * NGx];
        dT_spin = (0 == 0) ? d_spin[0 + (NGy - 1) * NGx] : d_spin[0 + (0 - 1) * NGx];
        dB_spin = (0 == NGy - 1) ? d_spin[0 + (0) * NGx] : d_spin[0 + (0 + 1) * NGx];
        metro_gmem_even<<<blocks, threads >> > (d0_spin, dT_spin, dB_spin, dL_spin, dR_spin, d_rng[0], B, T);


// test code end

        // test code start
        printf("Lx Ly NX: %d %d %d\n", Lx, Ly, Nx);
        cudaSetDevice(Dev[0]);
        for (int i = 0; i < Ly; i++) {
            int *h, *d;
            h = spin + 0 * Lx + (0 * Ly + i) * Nx;
            d = d_spin[0] + i * Lx;
        ...
        for (int i = 0; i < Ly; i++) {
            int *h, *d;
            h = spin + 1 * Lx + (0 * Ly + i) * Nx;
            d = d_spin[1] + i * Lx;
            cudaMemcpy(h, d, Lx * sizeof(int), cudaMemcpyDeviceToHost);
        }

        FILE *output_1 = fopen("0.dat", "w");
        for (int i = 0; i < Ny; ++i) {
            for (int j = 0; j < Nx; ++j) {
                fprintf(output_1, "%d\n", spin[i * Nx + j]);  // save the raw data
            }
            fprintf(output_1, "\n");  // save the raw data
        }
        fclose(output_1);
        printf("Current sweep: %d\n", swp);
        fflush(stdout);
        // FIXME second GPU's calculation is weird
        getchar();
        // test code end
...
/*
    #pragma omp parallel private(cpu_thread_id)
    {
      ...
      #pragma omp barrier
	  printf("CPU %d info: cpuid_x cpuid_y device: %d %d %d\n", cpu_thread_id, cpuid_x, cpuid_y, Dev[cpu_thread_id]);

      int *dL_spin, *dR_spin, *dT_spin, *dB_spin, *d0_spin;
      d0_spin = d_spin[cpu_thread_id];
      cpuid_x = 1-cpuid_x;// change pos at left right
