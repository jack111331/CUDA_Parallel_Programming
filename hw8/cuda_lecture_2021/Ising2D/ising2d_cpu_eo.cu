...
int main(void) {
  ...
  int *spin;            // array of spin variables
  int parity;           // parity of the even-odd indexing
  ...
  FILE *output;            
  output = fopen("ising2d_cpu_eo.dat","w");
  FILE *output3;
  output3 = fopen("spin_cpu_eo.dat","w");   
...
//
    for(int io=0; io<ns/2; io++) {     // update odd sites
      x = (2*io)%nx; 
      y = ((2*io)/nx)%nx;
      parity=(x+y+1)%2;
      x = x + parity;
