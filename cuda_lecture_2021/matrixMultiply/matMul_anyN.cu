...
//      C[i] = 1.0/A[i] + 1.0/B[i];
	...
	// TODO initialize C matrix
    for (long i = blockDim.x * blockIdx.x + threadIdx.x; i < N; i += blockDim.x * gridDim.x) {
	    for (long j = blockDim.y * blockIdx.y + threadIdx.y; j < N; j += blockDim.y * gridDim.y) {
	        C[i][j] += A[i] + 1.0/B[i];
        }
    }
...
void *allocateMatrixMemory(long N) {
