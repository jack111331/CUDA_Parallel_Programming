...
__global__ void hist_gmem(const float *data, const long N, unsigned int *hist, 
                          const int bins, const float Rmin, const float Rmax, const float binsize) 
{
...
int main(void)
{
    ...
    printf("%d\n",bins);
    fflush(stdout);
    int bsize = bins*sizeof(unsigned int);
...
//    RandomUniform(data_h, N);      // uniform deviate in (0,1)
      RandomExponential(data_h, N);       // Exponential distribution between [0, inf]
//    RandomNormal(data_h, N);       // Gaussian deviate with sigma = 1
    ...
    hist_gmem <<< blocksPerGrid, threadsPerBlock >>> (data_d, N, hist_d, bins, Rmin, Rmax, binsize);
