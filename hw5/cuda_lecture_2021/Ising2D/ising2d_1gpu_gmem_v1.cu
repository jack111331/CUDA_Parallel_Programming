...
__global__ void metro_gmem(int* spin, float *ranf, const float B, const float T)
{
    ...
    // syncthreads only applies to all threads in a block,   
    // but NOT all threads in the grid. 
    // In other words, not all blocks are synchronized.
    // Thus even if two simulations start with the same RNG seed,
    // they do not necessarily get exactly the same result. 
    // One way to avoid this problem is to update even/odd sites
    // with two separate kernels, as in v2. Another way is to use the
    // method of cooperative groups, which is only available for GPUs  
    // with compute capability > sm_60 and compile with CUDA 9.1, 
    // as shown in v3.  
...
int main(void) {
  ...
  for(int swp=0; swp<nt; swp++) {      // thermalization
    ...
    metro_gmem<<<blocks,threads>>>(d_spin, d_rng, B, T);    // updating with Metropolis algorithm
  }
  ...
  for(int swp=nt; swp<sweeps; swp++) {
    ...
    rng_MT(h_rng, ns);                                  // generate ns random numbers 
    cudaMemcpy(d_rng, h_rng, ns*sizeof(float), cudaMemcpyHostToDevice);
    metro_gmem<<<blocks,threads>>>(d_spin, d_rng, B, T);
