...
// Create exactly the same hermition matrix for both a and r, from an
// initial random matrix a.

void hermitian(int n, double *a, double *r)
{
   int     i, j, ii, jj;
   double  ar, ai, br, bi;
   ...
   memcpy(a, r, n*n*2*sizeof(double));
}
...
// Randomize the matrix a and copy a -> r
   int  nn = n2*2;
   lapackf77_dlarnv(&ione, ISEED, &nn, a);
   hermitian(n, a, r);
